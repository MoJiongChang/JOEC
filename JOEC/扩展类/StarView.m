...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    for (int j = 0; j < 5; j ++)
    {
        ...
        imageView.frame = CGRectMake(2*j*imageW, 0, imageW+10, imageW+10);
...
-(void)tapGR:(UITapGestureRecognizer *)tapGR{
    ...
    self.starForegroundView.frame = CGRectMake(0, 0, (X+1)*2*imageW, imageW+10);
    [self addSubview:self.starForegroundView];
    
    
    [self.delegate KaoPuDusendStarNum:X+1];
    [self.delegate1 ZhuanYeZhiShusendStarNum:X+1];
    [self.delegate2 XueBaZhiShusendStarNum:X+1];
