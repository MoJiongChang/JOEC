...
#import "chatTimeLabel.h"

@interface chatTimeLabel ()
////显示小时
...
- (void)setCountDownTimeInterval:(NSTimeInterval)countDownTimeInterval
{
    ...
    self.text = [NSString stringWithFormat:@"聊天时间 %02d:%02d:%02d",_hour,_minute,_second];
...
- (void)adjustCoundDownTimer:(NSTimer *)timer
{
    ...
    _hour = _countDownTimeInterval / 3600;
     ...
     NSLog(@" %d _hour = %02d _minute = %02d _second = %02d",(int)_countDownTimeInterval,_hour,_minute,_second);
    
     self.text = [NSString stringWithFormat:@"聊天时间 %02d:%02d:%02d",_hour,_minute,_second];
    NSLog(@"label text %@",self.text);
...
{
    NSLog(@"_countDownTimeInterval %d",(int)_countDownTimeInterval);
