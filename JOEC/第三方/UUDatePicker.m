...
- (void)drawRect:(CGRect)rect
{
    if (self.frame.size.height<216 || self.frame.size.width<320)
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, 260, 150);
...
- (NSArray *)getNowDate:(NSDate *)date
{
    ...
    if (self.datePickerStyle == UUDateStyle_HourMinute)
        return @[hour,minute];
    if (self.datePickerStyle == UUDateStyle_Minute)
        return @[minute];
    return nil;
}
...
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x, 45, 60, 72)];
    label.text = name;
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    if (self.datePickerStyle == UUDateStyle_YearMonthDayHourMinute){
        if (isIOS7) {
            [self creatValuePointXs:@[@"60",@"115",@"170",@"225",@"280"]
                          withNames:@[@"年",@"月",@"日",@"时",@"分"]];
        }
        return 5;
    }
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
        if (isIOS7) {
            [self creatValuePointXs:@[@"70",@"150",@"210"]
                      withNames:@[@"年",@"月",@"日"]];
            }
        return 3;
    }
