...
-(void)setImageCollectionView
{
    ...
    //注册cell
    photoCellID=@"YCPhotoCell";
    [self.imageCollectionView registerClass:[YCPhotoCell class] forCellWithReuseIdentifier:photoCellID];
}
//设置页数标签
-(void)setPagesLabel
{
...
-(void)computeThumbnailsRect
{
    ...
    for (int i=0; i<self.imagesURLArr.count; i++) {
        ...
        //再次判断是不是高清图与缩略图对应
        if (i==thumbnailView.tag-1000) {
           //有这张缩略图就记录在屏幕窗口下的位置大小
           thumbnailRect=[[thumbnailView superview] convertRect:thumbnailView.frame toView:window];
        }else{
...
-(void)imageEnlarge
{
    ...
    for (int j=0; j<self.thumbnailsArr.count; j++){
        thumbnailView=self.thumbnailsArr[j];
        if (self.currentImageIndex+1000==thumbnailView.tag) {
            break;//找到了就结束
        }else{
            thumbnailView=nil;//没有找到
        }
...
{
    YCPhotoCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:photoCellID forIndexPath:indexPath];
    ...
    for (int j=0; j<self.thumbnailsArr.count; j++){
        thumbnailView=self.thumbnailsArr[j];
        if (indexPath.row+1000==thumbnailView.tag) {
            break;//找到了就结束
        }else{
            thumbnailView=nil;//没有找到
        }
